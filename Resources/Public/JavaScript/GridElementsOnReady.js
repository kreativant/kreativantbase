...
define(['jquery', 'TYPO3/CMS/Backend/AjaxDataHandler', 'TYPO3/CMS/Backend/Storage', 'TYPO3/CMS/Gridelements/GridElementsDragDrop', 'TYPO3/CMS/Backend/LayoutModule/Paste', 'TYPO3/CMS/Backend/Modal', 'TYPO3/CMS/Backend/Severity'], function ($, AjaxDataHandler, Storage, DragDrop, Paste, Modal, Severity) {
        ...
        $('.t3-page-ce-wrapper-new-ce').each(function () {
            ...
            if (typeof(gridCell.data('allowed')) === 'undefined') {
                ...
                if (top.clipBoardElementTxGridelementsBackendLayout) {
                    if (typeof(gridCell.data('allowed-tx_kreativant_gridelements_backend_layout')) !== 'undefined') {
                        var allowedTxGridelementsBackendLayouts = gridCell.data('allowed-tx_kreativant_gridelements_backend_layout').toString().split(',');
                        if (
                            ...
                            || gridCell.data('allowed-tx_kreativant_gridelements_backend_layout') === '*'
                        ) {
                    ...
                    if (typeof(gridCell.data('disallowed-tx_kreativant_gridelements_backend_layout')) !== 'undefined') {
                        var disallowedTxGridelementsBackendLayouts = gridCell.data('disallowed-tx_kreativant_gridelements_backend_layout').toString().split(',');
                        if (
                            ...
                            || gridCell.data('disallowed-tx_kreativant_gridelements_backend_layout') === '*'
                        ) {
                            gridCell.data('allowedTxGridelementsBackendLayout', false);
                        }
                    }
    ...
    Paste.activatePasteModal = function (element) {
        ...
        if ($element.hasClass('t3js-paste-copy')) {
            var content = TYPO3.lang['tx_kreativant_gridelements_js.modal.pastecopy'] || '1 How do you want to paste that clipboard content here?';
            var buttons = [
                {
                ...
                {
                    text: TYPO3.lang['tx_kreativant_gridelements_js.modal.button.pastecopy'] || 'Paste as copy',
                ...
                {
                    text: TYPO3.lang['tx_kreativant_gridelements_js.modal.button.pastereference'] || 'Paste as reference',
                    btnClass: 'btn-' + top.TYPO3.Severity.getCssClass(severity),
                    trigger: function (ev) {
                        Modal.currentModal.trigger('modal-dismiss');
        ...
        $('table.t3js-page-columns > tbody > tr > td').each(function () {
            ...
            if (typeof colPos !== 'undefined') {
                if (typeof top.pageColumnsAllowed[colPos] !== 'undefined') {
                    ...
                    $(this).attr('data-allowed-tx_kreativant_gridelements_backend_layout', top.pageColumnsAllowed[colPos]['tx_kreativant_gridelements_backend_layout']);
                }
                if (typeof top.pageColumnsDisallowed[colPos] !== 'undefined') {
                    ...
                    $(this).attr('data-disallowed-tx_kreativant_gridelements_backend_layout', top.pageColumnsDisallowed[colPos]['tx_kreativant_gridelements_backend_layout']);
                }
        ...
        pageColumn.find('.t3js-page-new-ce:not(".t3js-page-new-ce-allowed") a').each(function () {
            if (typeof $(this).attr('href') !== 'undefined') {
                $(this).attr('href', $(this).attr('href').replace(
                    '&uid_pid',
                    (top.pageColumnsAllowed[colPos] ? ('&tx_kreativant_gridelements_allowed=' + window.btoa(JSON.stringify(top.pageColumnsAllowed[colPos]))) : '') +
                    (top.pageColumnsDisallowed[colPos] ? ('&tx_kreativant_gridelements_disallowed=' + window.btoa(JSON.stringify(top.pageColumnsDisallowed[colPos]))) : '') +
                    '&uid_pid'
                ));
            }
        });
