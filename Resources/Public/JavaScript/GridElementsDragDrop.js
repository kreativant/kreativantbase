...
define(['jquery', 'jquery-ui/droppable', 'TYPO3/CMS/Backend/LayoutModule/DragDrop'], function ($, Droppable, DragDrop) {
    ...
    DragDrop.onDragStart = function ($element) {
        ...
        if ($element.closest(DragDrop.newContentElementWizardIdentifier).length === 0) {
            ...
            if (typeof DragDrop.newContentElementOnclick !== 'undefined') {
                ...
                if (DragDrop.newContentElementOnclick.length) {
                    ...
                    DragDrop.types.data('tx_kreativant_gridelements_backend_layout', DragDrop.newContentElementDefaultValues.tx_kreativant_gridelements_backend_layout);
                }
            }
        }
        ...
        var gridType = DragDrop.types.data('tx_kreativant_gridelements_backend_layout');
        ...
        $(DragDrop.dropZoneIdentifier).not(DragDrop.ownDropZone).each(function () {
            ...
            var allowedGridType = closestColumn.data('allowed-tx_kreativant_gridelements_backend_layout') ? closestColumn.data('allowed-tx_kreativant_gridelements_backend_layout').toString().split(',') : null;
            var disallowedGridType = closestColumn.data('disallowed-tx_kreativant_gridelements_backend_layout') ? closestColumn.data('disallowed-tx_kreativant_gridelements_backend_layout').toString().split(',') : null;
            if ((
    ...
    DragDrop.onDrop = function ($draggableElement, $droppableElement, evt, reference) {
        ...
        if (contentElementUid > 0 || (DragDrop.newContentElementDefaultValues.CType && !$pasteAction)) {
            ...
            if (DragDrop.newContentElementDefaultValues.CType) {
                ...
                parameters['data']['tt_content']['NEW234134']['tx_kreativant_gridelements_container'] = container;
                parameters['data']['tt_content']['NEW234134']['tx_kreativant_gridelements_columns'] = gridColumn;
                parameters['data']['tt_content']['NEW234134']['sys_language_uid'] = language;

                if (!parameters['data']['tt_content']['NEW234134']['header']) {
                    parameters['data']['tt_content']['NEW234134']['header'] = TYPO3.l10n.localize('tx_kreativant_gridelements_js.newcontentelementheader');
                }
                ...
                parameters['cmd']['tt_content'][contentElementUid] = {
                    copy: {
                        ...
                        update: {
                            colPos: colPos,
                            tx_kreativant_gridelements_container: container,
                            tx_kreativant_gridelements_columns: gridColumn
                        }
                    }
                };
